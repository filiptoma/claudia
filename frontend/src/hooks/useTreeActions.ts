...
export function useTreeActions() {
  ...
  async function newFolder(p: Project): Promise<Folder | null> {
    try {
      const name = await dialog.prompt({ title: 'New folder', label: 'Folder name', confirmText: 'Create' })
      if (!name) return null
  ...
  async function newDocument(p: Project, folderId: string | null): Promise<DocumentRec | null> {
    try {
  ...
  async function submitDocTitle(d: DocMeta | DocumentRec, title: string): Promise<boolean> {
    const next = title.trim()
    if (!next || next === d.title) return false
    try {
      await renameDocument(d.id, next)
      qc.setQueryData<DocumentRec>(treeKeys.document(d.id), (old) => (old ? { ...old, title: next } : old))
      await refresh()
      toast('success', d.is_quick_note ? 'Quick note renamed' : 'Document renamed')
      return true
    } catch (e) {
      fail(e)
      return false
  ...
  async function editDocument(d: DocMeta | DocumentRec): Promise<string | null> {
    ...
    try {
      ...
      if (title && title !== d.title) {
        ...
        qc.setQueryData<DocumentRec>(treeKeys.document(d.id), (old) => (old ? { ...old, title } : old))
        await refresh()
  ...
  async function deleteDocument(d: DocMeta | DocumentRec): Promise<boolean> {
    ...
    try {
      ...
      await refresh()
      toast('success', isNote ? 'Quick note deleted' : `Document “${d.title}” deleted`)
      return true
    } catch (e) {
      fail(e)
      return false
