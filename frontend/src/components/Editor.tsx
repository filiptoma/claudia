...
function computeEditCapture(
  ...
  while (
    ...
    start < suggested.length &&
    original[start] === suggested[start]
  )
    start++;
  ...
  while (
    endOrig > start &&
    endSugg > start &&
    original[endOrig - 1] === suggested[endSugg - 1]
  ) {
    endOrig--;
    endSugg--;
  }
...
const COMMANDS: CommandDef[] = [
  {
    id: "ref",
    label: "Reference",
    sub: "Link to a document or heading",
    keywords: "reference ref link document doc heading anchor",
  },
];
...
export default function Editor({
  ...
  const handleSubmitSuggestion = useCallback(
    async (suggestedMd: string, note: string, mentions: string[]) => {
      ...
      try {
        ...
        toast(
          "error",
          e instanceof Error ? e.message : "Failed to submit suggestion",
        );
      } finally {
  ...
  const uploadAndInsert = useCallback(
    async (file: File, pos?: number) => {
      ...
      try {
        ...
        if (idx >= 0)
          view.dispatch({
            changes: { from: idx, to: idx + token.length, insert: md },
          });
      } catch (e) {
        const idx = view.state.doc.toString().indexOf(token);
        if (idx >= 0)
        ...
        toast(
          "error",
          "Image upload failed: " + (e instanceof Error ? e.message : "error"),
        );
      }
    },
    [doc.project_id],
  ...
  const syncSlash = useCallback((u: ViewUpdate) => {
    ...
    setSlash((prev) => ({
      ...
      coords: coords
        ...
        : (prev?.coords ?? { left: 0, top: 0, bottom: 0 }),
    }));
    ...
    () => [
      ...
      EditorView.domEventHandlers({
        drop: (event, view) => {
          ...
          const image = Array.from(files).find((f) =>
  ...
  const stepImageWidth = useCallback((dir: 1 | -1) => {
    ...
    if (base == null) {
      ...
      const el =
        idx >= 0
          ? previewRef.current?.querySelectorAll<HTMLImageElement>(".md img")[
              idx
            ]
          : undefined;
    ...
    const rel = Math.min(
      Math.max(0, view.state.selection.main.head - tok.from),
      md.length,
    );
    view.dispatch({
      changes: { from: tok.from, to: tok.to, insert: md },
      selection: EditorSelection.cursor(tok.from + rel),
    });
    view.focus();
  }, []);
      ...
      >
        <div
          className={cn(
            ...
            showPreview
              ? "flex-1 basis-1/2 border-r border-border max-md:border-r-0 max-md:border-b"
              : "flex-1",
          )}
          onBlur={() => {
            if (!suggestionMode) void doSave();
          }}
        >
      ...
      {suggestionMode &&
        ...
        (isMobile ? (
          ...
          <div
            ...
            style={{
              top: SPLIT_FLOATING_TOP,
              maxHeight: `calc(100vh - ${SPLIT_FLOATING_TOP}px - 1rem)`,
            }}
      ...
      <div className="flex min-h-0 min-w-0 flex-1 basis-1/2 flex-col">
        ...
        <div ref={bindScroll} className="min-h-0 flex-1 overflow-y-auto">
          <div
            ...
            //
            // When the comments/suggestions sidebar opens it pushes this whole pane narrower, so we drop
            // the 5/6 inset and the max-width cap and shrink to a small fixed side padding — letting the
            // prose reclaim the squeezed width instead of getting double-narrowed. Animated for a smooth
            // reflow as the rail slides in/out.
            className={cn(
