...
function PromptDialog({
  ...
  const schema = z.object({
    value: z
      .string()
      .trim()
      .min(1, `${opts.label ?? "This field"} is required`),
  });
  const {
    register,
    handleSubmit,
  ...
  const [selected, setSelected] = useState(
    choice?.choiceDefault ?? choice?.choices[0]?.value ?? "",
  );
    ...
    >
      <form
        ...
        onSubmit={handleSubmit(({ value }) =>
          onSubmit(value.trim(), choice ? selected : undefined),
        )}
        noValidate
      >
        {opts.label && (
          <Label htmlFor="claudia-prompt-input">{opts.label}</Label>
        )}
        <Input
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      active
                        ? "border-accent2 bg-accent2/10 text-foreground"
...
export function DialogProvider({ children }: { children: ReactNode }) {
  ...
  const promptWithChoice = useCallback((o: PromptWithChoiceOpts) => {
    ...
    setState({
      kind: "prompt",
      opts,
      choice: { choices, choiceLabel, choiceDefault, choiceNotices },
    });
      ...
      {state?.kind === "prompt" && (
        <PromptDialog
          ...
          onSubmit={(value, choice) =>
            settle(
              state.choice
                ? ({ value, choice: choice ?? "" } as PromptResult)
                : value,
            )
          }
        />
