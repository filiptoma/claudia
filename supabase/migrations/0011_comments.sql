-- 0011 — inline comments: threads anchored to a text range, with replies, resolve, @mentions.
-- Apply after 0001–0010. (0009/0010 are feedback/profile RPCs, unrelated to this feature.)
...
-- Is this user @mentionable in this project? (owner or any member.)
...
-- Integrity: project_id is ALWAYS the document's real project (runs before the RLS WITH CHECK).
...
-- Fail-closed mention validation.
...
-- @mention candidates (owner + members). Callable by anyone with view access. Includes avatar_url.
...
-- Create thread + first message atomically; project_id derived from the document.
...
-- Threads of a document with author + resolver display info.
...
-- Messages across a document's threads with author display info.
